  ...
  <div>
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      ...
      <div class="jarviswidget jarviswidget-color-darken" role="widget">
        ...
        <div v-if="profile.map['employees'].includes('index')" role="content">
            ...
            <div class="widget-body-toolbar">
              ...
              <div class="row">
                ...
                <div class="col-xs-3 col-sm-7 col-md-7 col-lg-7 text-right">
                  ...
                  <button v-if="profile.map['employees'].includes('create')" @click.prevent="$router.push(`/${basePath}/create`)" class="btn btn-success">
              ...
              <table class="table table-bordered table-striped table-condensed table-hover has-tickbox">
                ...
                <tbody>
                  <tr v-for="model in models">
                    ...
                    <td>{{ model['name'] }}</td>
                    <td>{{ model['profile']['description'] }}</td>
                    <td>{{ model['login'] }}</td>
                    <td>{{ model['email'] }}</td>
                    <td>
                      <span class="label" :class="{
                      'label-success': model['active'],
                      'label-danger': !model['active']}">
                    ...
                    <td>
                      <div class="pull-right">
                        <router-link v-if="profile.map['employees'].includes('show')" :to="{
                        path: `${basePath}/${model.id}/show`
                      }" class="btn btn-sm btn-default">
                      <i class="fa fa-external-link"></i> Ver detalhes
                    </router-link>
                    <router-link v-if="profile.map['employees'].includes('edit')" :to="{path: `${basePath}/${model.id}/edit`}" class="btn btn-sm btn-default">
                      <i class="fa fa-pencil"></i> Editar
                    </router-link>
                    <a v-if="profile.map['employees'].includes('delete')" @click.prevent="destroy(model.id)" class="btn btn-sm btn-danger">
