<template>
  <form id="main-form" class="smart-form">
    ...
    <div class="col col-md-12">
      <fieldset>
        <legend><i class="fa fa-dollar"></i> Aquisição</legend>
        <div class="row">
    ...
    <div class="col col-md-12">
      <fieldset>
        <legend><i class="fa fa-exchange"></i> ICMS</legend>
        <div class="row">
          <template v-for="field in __defaultFieldICMSSection">
            <component
            :is="field.component"
            :wrapper-class="field.wrapperClass"
            :data-vv-as="field.label"
    ...
    <div class="col col-md-12">
      <fieldset>
        <legend><i class="fa fa-exchange"></i> PIS</legend>
        <div class="row">
          <template v-for="field in __defaultFieldPISSection">
            <component
            :is="field.component"
            :wrapper-class="field.wrapperClass"
            :data-vv-as="field.label"
    ...
    <div class="col col-md-12">
      <fieldset>
        <legend><i class="fa fa-exchange"></i> COFINS</legend>
        <div class="row">
          <template v-for="field in __defaultFieldCOFINSSection">
            <component
            :is="field.component"
            :wrapper-class="field.wrapperClass"
            :data-vv-as="field.label"
...
<script>
  ...
  import {
    ...
    __defaultFieldICMSSection,
    __defaultFieldPISSection,
    __defaultFieldCOFINSSection
  } from './../metadata'
  ...
  export default {
    ...
    computed: {
      ...
      __defaultFieldICMSSection () {
        return __defaultFieldICMSSection
      },
      __defaultFieldPISSection () {
        return __defaultFieldPISSection
      },
      __defaultFieldCOFINSSection () {
        return __defaultFieldCOFINSSection
      }
    },
    watch: {
      year (value) {
    ...
    created () {
      ...
      this.$root.axios.get('get-config?icms_rate&icms_basetax_st_reduction&icms_basetax_modality&icms_cst&icms_origin&pis_rate&pis_cst&cofins_rate&cofins_cst').then(({data}) => {
        data.forEach(item => {
          this[item.key] = item.value
        })
      }).catch(err => {
        alert('Ocorreu um erro ao buscar as configurações de nota fiscal')
        throw new Error(err)
      })
    ...
    methods: {
      ...
      create () {
        this._data.tenant_id = localStorage.getItem('current-tenant-idkey')
        this.$validator.validateAll().then(() => {
          this.api.create(this._data, ({ data }) => {
            this.id = data.id_key
            this.$router.push(`/vehicle-entry/${data.id}/edit`)
          })
        }).catch(() => {})
      },
      destroy () {
