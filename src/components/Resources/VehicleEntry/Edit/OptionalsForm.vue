        ...
        <section v-for="optionalItem in searchedOptionalItems" class="col col-md-6">
          <div>
            <label :class="{'toogle-active': selectedOptionalItems.includes(optionalItem.id_key)}" class="toggle">
              <input :disabled="$parent.$refs.customFormPersonalData && $parent.$refs.customFormPersonalData.unidadesDiferentes" @change="clearAndFocusSearch" v-model="selectedOptionalItems" :value="optionalItem.id_key" type="checkbox" name="name">
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      // console.log(this.$parent.$refs.customFormPersonalData.id)
