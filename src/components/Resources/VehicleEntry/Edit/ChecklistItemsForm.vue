        ...
        <section v-for="checklistItem in searchedChecklistItems">
          <div class="col-md-4">
            <label :class="{'toogle-active': selectedChecklistItems.includes(checklistItem.id)}" class="toggle">
              ...
              <input :disabled="$parent.$refs.customFormPersonalData && $parent.$refs.customFormPersonalData.unidadesDiferentes"  @change="clearAndFocusSearch" v-model="selectedChecklistItems" :value="checklistItem.id" type="checkbox" name="name">
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.$root.axios.get(`checklist-item-entries`, {
        params: {
          q: `entry_id:${this.$parent.$refs.customFormPersonalData.id}`,
          sf: `entry_id:=`
        }
      }).then(({ data }) => {
        this.selectedChecklistItems = map(data.data.data, 'checklist_item_id')
      })
    },
    computed: {
      currentTenant () {
